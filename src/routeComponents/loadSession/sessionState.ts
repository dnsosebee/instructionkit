...
type SessionTypestate =
  | {
      value: 'loadingSession' | 'signedOut'
      context: {
        session: undefined
        profile: undefined
      }
  ...
  | {
      value: { loadedSession: 'loadingProfile' }
      context: {
        session: Session
        profile: undefined
      }
...
export const sessionStateMachine =
  ...
  createMachine<SessionContext, SessionEvent, SessionTypestate>(
    {
    ...
    },
    {
      actions: {
        setSession: assign({
          session: (_, event) => (event.type === 'SIGNED_IN' ? event.session : undefined),
        }),
        setProfile: assign({
          profile: (_, event) => {
            if (event.type === 'REGISTERED') return event.profile
            if (event.type === 'UNREGISTERED') return event.profile
            return undefined
          },
        }),
      },
